...
use {
    super::{
        ...
        edit::Edit,
        preedit_range, preedit_text, AttrsExt,
    },
    core::{iter::once, ops::Range},
    cosmic_text::{
        Attrs, AttrsList, BufferLine, BufferRef, Change, ChangeItem, Color, Cursor, FontSystem,
        LayoutRun, Selection, Shaping, SwashCache,
    },
    std::cmp,
            ...
            for run in buffer.layout_runs() {
                ...
                if !cursor_hidden {
                    if let Some((x, y)) = cursor_position(&self.cursor, &run) {
                        f(x, y, 1, line_height as u32, style.cursor_color);
                    }
                }
                ...
                for glyph in run.glyphs.iter() {
                    ...
                    if style.text_color != style.selected_text_color {
                        if let Some((start, end)) = selection_bounds {
                            ...
                            {
                                glyph_color = style.selected_text_color;
                            }
                        }
                    }
...
impl<'buffer> Edit<'buffer> for Editor<'buffer> {
    ...
    fn preedit_range(&self) -> Option<Range<usize>> {
        self.with_buffer(|buffer| preedit_range(&buffer.lines[self.cursor.line]))
    }

    fn preedit_text(&self) -> Option<String> {
        self.with_buffer(|buffer| preedit_text(&buffer.lines[self.cursor.line]).map(Into::into))
    }
        ...
        match action {
            Action::Motion { motion, select } => {
                ...
                if select {
                    ...
                    if start.line != end.line || start.index != end.index {
                        let new_cursor = match motion {
                            ...
                            Motion::Other(cosmic_text::Motion::Previous) => Some(start),
                            Motion::Other(cosmic_text::Motion::Next) => Some(end),
                            Motion::Other(cosmic_text::Motion::Left) => self
                            ...
                            Motion::Other(cosmic_text::Motion::Right) => self
                                .with_buffer_mut(|buffer| {
                                    buffer
                                        .line_shape(font_system, cursor.line)
                                        .map(|shape| shape.rtl)
                ...
                if let Some((new_cursor, new_cursor_x_opt)) =
                    self.with_buffer_mut(|buffer| match motion {
                        ...
                        Motion::DocumentEnd => {
                            let mut new_cursor = cursor;
                            new_cursor.line = buffer.lines.len() - 1;
                            new_cursor.index = buffer.lines.get(new_cursor.line)?.text().len();
                            Some((new_cursor, None))
                        }
                        Motion::Other(motion) => {
            ...
            Action::DeleteStartOfWord => {
                if self.delete_selection() {
                    ...
                    self.action(
                        font_system,
                        Action::Motion {
                            motion: Motion::Other(cosmic_text::Motion::PreviousWord),
                            select: true,
                        },
                    );
            ...
            Action::DeleteEndOfWord => {
                if self.delete_selection() {
                    ...
                    self.action(
                        font_system,
                        Action::Motion {
                            motion: Motion::Other(cosmic_text::Motion::NextWord),
                            select: true,
                        },
                    );
                    self.delete_selection();
